...
### 事务
所有的指令在exec
之前不执行，而是缓存在服务器的一个事务队列中，服务器一旦收到exec指令，才开始事务队列执行。因为redis为单线程，不用担心执行时被其他指令打搅。

redis事务不具备原子性，如果指令执行失败，后面的指令还会继续执行。

redis的客户端在执行事务时都会结合Pipeline一起使用。

redis提供watch机制，它就是一种乐观锁。会在事务开始之前盯住一个关键变量，当事务执行时，也就是服务器收到了exec指令要顺序执行缓存的事务队列时，redis会检查关键变量自watch之后是否被修改了。如果关键变量被人动过，exec指令就会返回null回复告知客户端事务执行失败，这个时候客户端一般会选择重试。

注意：redis禁止在multi和exec之间进行watch操作

### PubSub
因为用list做消息队列不支持消息多播，这就诞生了PubSub

redis提供模式订阅功能的Pattern Subscribe，这样可以一次订阅多个主题。生产者新增加了同模式的主题，消费者也可以立即收到消息。

但是，如果一个消费者都没有，消息就会被直接丢弃。而且PubSub的消息也不会持久化。

## 对象压缩
如果redis内部管理的集合数据结构很小，会使用紧凑存储形式压缩存储。当集合对象的元素不断增加，或者某个value过大，对象存储会被升级为标准结构。

redis并不总是将空闲内存立即归还给操作系统。操作系统是以页为单位回收内存的。只要这个页有一个key在使用，它就不能回收。
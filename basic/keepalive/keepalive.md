...
* **负载均衡**: 阿里云SLB（7层结构）
* **异常现象**: 服务端连接数过多，出现异常
* **异常处理**: 将客户端调用由长连接改为短连接后，连接数下降，压力得到缓解
...
### 再看问题
到这儿，我们算是了解了HTTP长短连接的使用方式和原理，可以看到一般情况下长连接都会有比短连接更好的表现。但问题依然得不到解答：为什么压测中使用短连接却有“更好”的表现呢，难道我们忽略了什么

对，就是阿里云的slb

在仿真环境进行测后发现，不管使用长连接还是短连接，slb对后端业务都是使用长连接。当HTTP请求响应完成后，客户端则与负载均衡设备协商关闭连接，而负载均衡则保持与服务器之间的这个连接，直到设定后的超时时间后再关闭。换句话说：我们设定的长/短连接，其实对服务端业务并没有影响（但可能会影响客户端的TIME_WAIT状态）

由于出现异常当时的数据并没有记录，而且客户端做出调整的地方不止一处，我们现在很难判断当时到底哪个修改影响了最后的结果。但从上面的分析可以看到，没有进行对照试验，改为短连接后问题得到缓解，然后认定长连接有问题，是比较草率的。而且修改后影响了多少，降低了多少也没有数据支持，只能在以后的压测中验证
